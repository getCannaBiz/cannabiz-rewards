<?php
/**
 * Main Class file for `CannaBiz_Rewards_OSA`
 *
...
if ( ! class_exists( 'CannaBiz_Rewards_OSA' ) ) :
    ...
    class CannaBiz_Rewards_OSA {
        ...
        function callback_checkbox( $args ) {
            ...
            $html  = '<fieldset>';
            $html .= sprintf( '<label for="cannabiz-rewards-%1$s[%2$s]">', $args['section'], $args['id'] );
            $html .= sprintf( '<input type="hidden" name="%1$s[%2$s]" value="off" />', $args['section'], $args['id'] );
            $html .= sprintf( '<input type="checkbox" class="checkbox" id="cannabiz-rewards-%1$s[%2$s]" name="%1$s[%2$s]" value="on" %3$s />', $args['section'], $args['id'], checked( $value, 'on', false ) );
        ...
        function callback_multicheck( $args ) {
            ...
            foreach ( $args['options'] as $key => $label ) {
                ...
                $html   .= sprintf( '<label for="cannabiz-rewards-%1$s[%2$s][%3$s]">', $args['section'], $args['id'], $key );
                $html   .= sprintf( '<input type="checkbox" class="checkbox" id="cannabiz-rewards-%1$s[%2$s][%3$s]" name="%1$s[%2$s][%3$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked( $checked, $key, false ) );
        ...
        function callback_radio( $args ) {
            ...
            foreach ( $args['options'] as $key => $label ) {
                $html .= sprintf( '<label for="cannabiz-rewards-%1$s[%2$s][%3$s]">', $args['section'], $args['id'], $key );
                $html .= sprintf( '<input type="radio" class="radio" id="cannabiz-rewards-%1$s[%2$s][%3$s]" name="%1$s[%2$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked( $value, $key, false ) );
                $html .= sprintf( '%1$s</label><br>', $label );
            }
            $html .= $this->get_field_description( $args );
            $html .= '</fieldset>';
